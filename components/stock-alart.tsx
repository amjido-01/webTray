...
export function StockAlertTable({ data }: DataTableProps) {
      ...
      <div>
        <Card className="border-0 shadow-none">
          <CardHeader className="flex flex-row items-center justify-between">
            ...
            <Button variant="ghost" size="sm" className="rounded-full border-[1px] bordeer-gray-300" onClick={() => setViewAllModal(true)}>
              View All
            </Button>
          </CardHeader>
          <CardContent>
              ...
              {stockAlerts.map((item, index) => (
                <div key={index} className="flex items-center justify-between border-b">
                  <div>
                    ...
                    <p className="text-sm text-gray-500">{item.units} units remaining</p>
                  </div>
                  <Badge className={getStockLevelColor(item.level)}>{item.level}</Badge>
                </div>
              ))}
            </div>
          </CardContent>
        </Card>
        <StockAlertsModal isOpen={viewAllModal} onOpenChange={setViewAllModal} stockAlerts={ stockAlerts } />
      </div>
    </div>
  )
}
